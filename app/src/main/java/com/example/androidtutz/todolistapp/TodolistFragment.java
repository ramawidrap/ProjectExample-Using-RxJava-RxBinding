...
public class TodolistFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        disposable.add(RxTextView.textChangeEvents(searchEditText).skipInitialValue().debounce(3000000, TimeUnit.MICROSECONDS)
                .distinctUntilChanged().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new DisposableObserver<TextViewTextChangeEvent>() {
                    ...
                    public void onNext(TextViewTextChangeEvent value) {
                        goalAdapter.getFilter().filter(value.text());
                    }
    ...
    private void loadData() {
        ...
        disposable.add(observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).filter(toDoListItem -> toDoListItem.getToDoListItemStatus().equalsIgnoreCase(taskStatus)).map(it -> {
            it.setToDoListItemDescription(dateFront + " " + it.getToDoListItemPlanedAchievDate());
            return it;
